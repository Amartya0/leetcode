...
class MyCircularDeque:
    def __init__(self, k: int):
        ...
        self.size = k+1
        self.dataArr = [None]*(k+1)

    def insertFront(self, value: int) -> bool:
        if self.isFull():
            return False
        self.dataArr[self.front] = value
    ...
    def getFront(self) -> int:
        if self.isEmpty():
            return -1
        return self.dataArr[(self.front-1+self.size) % self.size]

    def getRear(self) -> int:
        if self.isEmpty():
            return -1
        return self.dataArr[self.rear]
