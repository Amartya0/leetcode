...
class Solution:
    def maximumBeauty(self, items: list[list[int]], queries: list[int]) -> list[int]:
        ...
        price_arr, beauty_arr = [], []
        max_beauty = 0

        for price, beauty in items:
            max_beauty = max(max_beauty, beauty)
            if not price_arr or price_arr[-1] != price:
                price_arr.append(price)
                beauty_arr.append(max_beauty)
            else:
                beauty_arr[-1] = max(beauty_arr[-1], beauty)

        result = []

        for q in queries:
            idx = bisect.bisect_right(price_arr, q) - 1
            if idx != -1:
                result.append(beauty_arr[idx])
            else:
                result.append(0)
